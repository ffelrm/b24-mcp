...
КОНТРАКТ (снят эмпирически на живом облачном портале, июнь 2026):
...
- Легаси `task.commentitem.getlist` показывает ТОЛЬКО старые форумные комменты и
  отстаёт: на одной из долгоживущих задач он обрывался на окт-2025, а в чате — живой обмен
...
def fetch_task_results(client: Client, task_id: int) -> list[dict[str, Any]]:
    """Результаты задачи — отдельная сущность, а НЕ комментарий чата.

    Результат ставится поверх задачи и несёт СВОЙ текст, которого в чате нет
    вовсе: туда уезжает только системная строка «добавил результат задачи». По
    ней содержимое неотличимо от «пометил старую реплику», хотя внутри может
    лежать отчёт о выкатке, спецификация или решение по спорному вопросу.

    `commentId: 0` означает «результат написан сам по себе»; ненулевой — что он
    сделан ИЗ комментария, и тогда текст обычно совпадает с той репликой.
    Спутав их, делаешь вывод, обратный происходившему.

    Возвращает список в порядке выдачи API (старые → новые), пустой — если
    результатов нет либо метод на портале недоступен.
    """
    try:
        res = client.call("tasks.task.result.list", {"taskId": int(task_id)})
    except B24Error:
        # Метода нет на портале / нет прав — это не отказ команды: у задачи
        # просто не будет блока результатов.
        return []
