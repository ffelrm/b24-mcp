...
class Config:
    ...
    # Опциональные поля на случай, если твой портал отдаёт часть данных не через
    # вебхук, а через собственный кастомный rest.php-action со своим hash
    # (отдельная авторизация, не путать с REST-вебхуком выше). У большинства
    # порталов этого слоя нет — тогда поля пустые и не используются.
    ...
    # Вебхук УМЕЕТ методы фоллоу-апов: путь /rest/api/<uid>/<token>/<method>
    # (префикс api/ идёт ПЕРЕД uid/token), нужен scope `call`. Отдельных кредов
    # не требуется — это основной транспорт. Поля ниже — опциональные АЛЬТЕРНАТИВЫ
    # на случай, если вебхук-путь на твоём портале недоступен: (a) OAuth локального
    # приложения со scope `call`; (b) кастомный rest.php-action со своим hash.
    # Если заданы — берутся вместо вебхука.
    ...
    followups_action: str = ""  # имя кастомного action на твоём портале (только транспорт b)
...
def load() -> Config:
    ...
    return Config(
        ...
        followups_action=g("FOLLOWUPS_ACTION") or "",
        env_path=str(chosen),
    )
