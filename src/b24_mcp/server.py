...
def task_result(
    ...
    """Результаты задачи — их НЕТ в чате, там только системная строка без текста.

    Читай, когда в переписке мелькнуло «добавил результат задачи»: содержимое
    может быть отчётом о выкатке, спецификацией или решением по спорному
    вопросу, и по чату этого не видно. `comment_id: 0` — результат написан сам
    по себе, ненулевой — сделан из комментария (тогда текст обычно повторяет
    ту реплику).
    """
    return api.task_result(_client(), task_id)
...
def main() -> None:
    ...
    logger.info("b24-mcp: старт (read-only, %d инструментов)", 14)
    mcp.run()
