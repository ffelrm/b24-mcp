...
from b24_mcp.client import B24Error  # noqa: E402
...
def test_task_result_reads_own_entity_not_the_chat():
    """Результат берётся своим методом: в чате его текста нет вовсе."""
    c = FakeClient({
        "tasks.task.get": {"task": {"id": "7", "title": "Задача", "chatId": "42"}},
        "tasks.task.result.list": [
            {"id": 3, "createdBy": 5, "createdAt": "2026-07-01T10:00:00+03:00",
             "commentId": 0, "text": "[B]итог[/B] выкатили"},
        ],
        "user.get": [{"ID": "5", "NAME": "Имя", "LAST_NAME": "Фамилия"}],
    })
    res = api.task_result(c, 7)
    assert [m for m, _ in c.calls].count("tasks.task.result.list") == 1
    assert res["count"] == 1
    item = res["items"][0]
    assert item["text"] == "итог выкатили"  # BB-разметка вычищена
    assert item["comment_id"] == 0  # написан сам по себе, не из реплики
    assert item["author"] == "Имя Фамилия"


def test_task_result_survives_missing_method():
    """Метода нет на портале → пустой блок результатов, а не падение вызова."""
    class Boom(FakeClient):
        def call(self, method, params=None):
            self.calls.append((method, params or {}))
            if method == "tasks.task.result.list":
                raise B24Error(method, "ERROR_METHOD_NOT_FOUND", "нет такого метода")
            return self.responses.get(method)

    c = Boom({"tasks.task.get": {"task": {"id": "7", "title": "Задача"}}})
    res = api.task_result(c, 7)
